...
export const DECISIONS: DecisionCategory[] = [
  {
    ...
    entries: [
      ...
      },
      {
        ...
        decision:
          "The contract is global recall ≥ 0.35 AND false-positive rate ≤ 0.10; both must hold for the verdict to pass. Recall ≥ 0.35 says at least a third of injected anomalies must be caught; FPR ≤ 0.10 says no more than 10% of clean store-days may be falsely flagged. These are platform-design thresholds set when the detection layer was specified — targets the static-band rules should clear against deterministically injected anomalies — not industry standards or a benchmark borrowed from elsewhere. The canonical now clears both — recall 1.0 and false-positive rate 0.009 — and [the detection-quality page](/about/detection-quality) publishes that passing verdict. The bar held the whole way: when an earlier canonical missed the FPR half, the fix was to improve the detector until it cleared the threshold, not to loosen the threshold to make the verdict green.",
        rejected:
        ...
        cost:
          "0.35/0.10 is a floor, not a quality ceiling — a detector can clear both halves while still missing much of what matters: recall ≥ 0.35 only asks for a third of injected anomalies, and the FPR ceiling says nothing about the severity of what slips through. The canonical now passes both halves comfortably (recall 1.0, FPR 0.009), but the contract still measures whether the detector clears a deliberately modest bar, not whether it is as sharp as a production detector would need to be.",
        revisitWhen:
          "Real retail data with seasonality and promotion calendars would force both numbers to be recalibrated against a higher false-positive baseline than synthetic data produces — the [baselines-for-real-data work](/about/operations#detection-calibration) the detection section flags as still open. And now that there's headroom (FPR sits at 0.009, well under the 0.10 ceiling), the floor and ceiling could both tighten to reflect the detector's measured performance rather than the modest bar it was first held to.",
        honestNote:
          "0.35 and 0.10 are round numbers I picked as a first bar, not values derived from a cost model weighing a missed integrity_breach against a false alarm. They're defensible — a third of anomalies caught, a tenth of clean days falsely flagged — but the real tradeoff was never quantified. A production detector would set these from that cost model, not from what looked reasonable when the layer was specified. The part I'd defend without hesitation: when an earlier canonical cleared recall but missed the FPR half, I closed the gap by adding rules shaped to the anomalies that were slipping through, not by sliding the bar down to where the detector already sat.",
      },
      {
