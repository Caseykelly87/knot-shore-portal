...
export const DECISIONS: DecisionCategory[] = [
  {
    ...
    entries: [
      ...
      },
      {
        ...
        cost:
          "The bands are deliberately wide because they don't adjust for day-of-week or seasonal variance. This produces some false positives on legitimate weekly cycles. Acceptable because the demo dataset is small and the detection contract is met. The YAML config notes this directly: a future baselines phase will provide empirical per-store-date expectations; at that point these static bands can be narrowed or replaced.",
        revisitWhen:
          "Real retail data with seasonality and promotion calendars would produce massive false-positive rates against these bands. At that point, an empirical-baseline phase is necessary.",
        honestNote:
          "ML-based detection here would have been engineering theater. The platform handles 8 stores and 184 days of synthetic data. The static bands are more honest than ML would have been, because the question is whether the bands' parameters match the simulator's parameters, which is a transparent test. ML would have hidden the question behind training data.",
      },
      {
