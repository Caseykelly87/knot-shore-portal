...
export const LESSONS: LessonEntry[] = [
  ...
  {
    title: "Rule of three caught late",
    whatHappened: [
      "The portal's /api/* routes follow a dual-mode pattern by design: read API_MODE, serve a bundled fixture in offline mode, proxy the upstream api in online mode, fall back to the fixture and set X-Data-Source: fallback when the upstream is unreachable. The pattern is a real architectural choice — the platform demos standalone without infrastructure — and the first /api/* route handler implemented it correctly.",
      "I wrote the second route handler by copying the first. Then the third. Then the fourth, fifth, and sixth. Each was ~70 lines, each looked almost identical to the one before it, each only differed in the path string, the upstream path, and the fixture loader function. By the time the sixth one shipped, the same shape existed six times across the routes/ directory. The rule-of-three threshold — the heuristic that says \"extract before writing the fourth copy\" — had been crossed quietly, three copies back.",
      "The pagination story was worse. lib/store-data.ts had the canonical 200-row paginator that walked the upstream's envelope. lib/store-metrics-loader.ts grew its own version with a slightly different shape. lib/exceptions-data-server.ts grew a third, also slightly different. lib/dashboard-data.ts got no paginator at all — instead it asked the upstream for limit=5000 in a single-shot fetch. The upstream caps limit at 200 and rejects anything larger with a 422.",
      "Offline mode masked the bug. The bundled fixtures don't enforce the 200-row cap, so the dashboard rendered correctly in the fixture path that the tests exercised. Online mode against a real api would have either crashed (422) or silently rendered only the first 200 rows of each capped endpoint — about 7% of the 2,944 store-days behind the dashboard's revenue. The contract test in tests/contract/api-portal-contract.test.ts loaded fixtures directly and never touched the online-mode fetcher path, so it was green the whole time.",
      "External code review caught the bug. It also caught the broader duplication that produced it.",
    ],
    howIHandledIt: [
      "Two extractions: lib/proxy-route.ts's makeProxyRoute factory for the route handler shape, lib/pagination.ts's fetchPaginated for the upstream walk. The six route files collapsed from ~70 lines each to ~7. The four pagination callsites all go through one implementation. lib/dashboard-data.ts and lib/store-metrics-loader.ts picked up real pagination in the same commit.",
      "I dropped the ANOMALIES_FETCH_LIMIT and STORE_METRICS_FETCH_LIMIT = 5000 constants entirely. Leaving them in place would have preserved the broken contract behind a different name. The constants existed only because the original author (me) assumed the upstream had no cap; the right shape was always to paginate.",
      "Wrote a parameterized handler test that exercises the four canonical behaviors per route — offline serves the fixture, online success proxies the upstream, online failure either degrades to the fixture or returns the structured 503 (the /api/health distinction), and x-request-id propagates. Five of the six routes had no handler test before; now all six do, and the next time a route handler grows it'll have a test scaffold to extend rather than a fresh page.",
      "Extended ModeIndicator with a third state. The X-Data-Source: fallback header was already set; nothing read it. A small per-request flag (lib/data-source-state.ts using React's cache()) lets server-side data fetchers mark when they observed the header and lets the indicator surface \"Live Data (Fallback)\" when they did. The order semantics aren't perfect — App Router's concurrent rendering means the indicator may lag by one render — but the common case works and the failure mode is the previous behavior, not worse.",
    ],
    whatItTaught: [
      "Extract at the third copy. The rule of three exists because two copies could be coincidence and four is too many to fix cheaply. At three, the shape is real enough to abstract and the cost of abstracting is still low. I wrote past three because each new copy felt cheaper than designing the abstraction would have been — a local-optimum trap. The cost shows up later, in the fourth, fifth, and sixth copies that all carry the same drift.",
      "Integration tests that share the implementation's escape hatch aren't testing the integration. The contract test loaded fixtures directly because fixtures were the offline-mode escape hatch, and offline mode hid the bug. A real integration test would have hit the online-mode path against a real or mocked upstream — at which point the 200-row cap would have surfaced the first time the suite ran. \"What does the test environment look like\" is a question worth asking explicitly when a contract test claims to pin the api → portal contract.",
      "Code reviewers — human or otherwise — find the bugs that the author can't see. I had stared at the six route handler files multiple times and never noticed the shape was repeating. The reviewer noticed in one pass. Some duplication is invisible to the person who wrote it because they wrote each instance in isolation; a second pair of eyes sees the pattern that's only visible at the directory level.",
      "When you find yourself reaching for a 5000-row limit on an api you don't fully understand, stop. The upstream's documented cap was 200. I'd built the api myself. I forgot. The right shape was always paginate-and-accumulate, the same shape the other three callsites already used. I wrote a worse fourth implementation because I wasn't looking at the other three when I wrote it.",
    ],
  },
  {
