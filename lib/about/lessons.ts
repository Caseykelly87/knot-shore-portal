...
export const LESSONS: LessonEntry[] = [
  ...
  },
  {
    ...
    whatHappened: [
      ...
      "Offline mode masked the bug. The bundled fixtures don't enforce the 200-row cap, so the dashboard rendered correctly in the fixture path that the tests exercised. Online mode against a real api would have either crashed (422) or silently rendered ~7% of revenue and ~22% of anomalies (200 of 894). The contract test in tests/contract/api-portal-contract.test.ts loaded fixtures directly and never touched the online-mode fetcher path, so it was green the whole time.",
