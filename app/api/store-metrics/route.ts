...
export async function GET(req: NextRequest) {
  ...
  log.info(
    { event: "route_started", path: "/api/store-metrics", mode },
    "route started",
  );

  let response: NextResponse;
  if (mode === "offline") {
    response = NextResponse.json(loadStoreMetricsFixture());
  } else {
    const upstream = `${getUpstreamBaseUrl()}/store-metrics${req.nextUrl.search}`;
    try {
  ...
  portalRequestsTotal
    .labels({
      route: "/api/store-metrics",
      mode,
      status_code: String(response.status),
    })
    .inc();
  portalRequestDurationSeconds
    .labels({ route: "/api/store-metrics", mode })
